...
// constants, variables
const { safeSync, safeAsync } = helper;
...
class jsoncSafe {
    ...
    static stringify(value: any, optionsOrReplacer?: IStringifyOptions | Replacer, space?: string | number): [Error, undefined] | [null, string] {
        const opts = helper.getStringifyOptions(optionsOrReplacer, space);
        try {
            return [null, fastSafeStringify(value, opts.replacer, opts.space)];
        } catch (err) {
     ...
     */
    static stripComments(str: string, whitespace: boolean = false): [Error, undefined] | [null, string] {
     ...
     *  // }
     */
    static beautify(str: string, space: string | number = 2): [Error, undefined] | [null, string] {
    ...
    static read(filePath: string, options?: IReadOptions): Promise<[Error, undefined] | [null, any]> {
        return safeAsync<any>(jsonc.read(filePath, options));
    }
    ...
    static write(filePath: string, data: any, options?: IWriteOptions): Promise<[Error, undefined] | [null, boolean]> {
        return safeAsync<boolean>(jsonc.write(filePath, data, options));
    }
